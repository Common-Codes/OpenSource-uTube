        ...
        store.collection("vid").where("channel", "==", channel)
            ...
            .then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
                    ...
                    const cuntent = `<div onclick="location.href='https://common-codes.github.io/OpenSource-uTube/watch/#vid=${vdata.id}'" class="video-data" style="position: relative; top: 64px;"><img src="${vdata.vod}" style="height: 90px; width: 150px; position: absolute; left: 5px;"><b class="video-title" style="position: absolute; left: 160px; top: 8px;">${vdata.title}</b></div><br>`;
